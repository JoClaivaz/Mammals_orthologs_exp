...
dat = read.table(text = "BOVIN GORGO MACMU MONDO MOUSE PANTR PIGXX
1 15591 16349 15438 10564 16051 16447 12485", header = T)
...
dat = read.table(text = "BOVIN GORGO MACMU MONDO MOUSE PANTR PIGXX
1 14605 15198 14418 9907 14990 15333 11648
2 822   986   830   492  892   994   647
3 164   165   190   165  169   120   190", header = T)
...
legend('bottomright', c("both", "partial", 'none'), fill = c('red','blue', 'green'), 
       cex = 0.6, horiz = T)

#analysis of the kind of modification amongst ortholog pairs (no modification/complex_modif/1 domain modification)
dat = read.table(text = "BOVIN GORGO MACMU MONDO MOUSE PANTR PIGXX
1 1006  880   997   884  881   722   974
2 512   339   378   468  433   258   460
3 13087 13979 13043 8555 13676 14353 10214", header = T)
...
dat = read.table(text = "BOVIN GORGO MACMU MONDO MOUSE PANTR PIGXX
1 472 399 480 424 392 300 544
2 534 481 517 460 489 422 430", header = T)
...
dat = read.table(text = "BOVIN GORGO MACMU MONDO MOUSE PANTR PIGXX
1 224786 232346 257597 183774 272425 206993 228663", header = T)
...
dat = read.table(text = "BOVIN GORGO MACMU MONDO MOUSE PANTR PIGXX
1 352 319 374 319 276 242 426", header = T)
b = barplot(as.matrix(dat), col = c('red'),
